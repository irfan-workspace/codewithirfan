      ...
      <div className="container mx-auto max-w-4xl">
        ...
        {experience.length > 0 && (
          <div className="mb-16">
            ...
            <div className="space-y-6">
              {experience.map(exp => (
                <motion.div key={exp.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="glass rounded-xl p-6">
        ...
        {testimonials.length > 0 && (
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {testimonials.map(t => (
                <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass rounded-xl p-6">
                  <p className="text-muted-foreground text-sm mb-4 italic">"{t.message}"</p>
                  <div>
                    <p className="font-medium text-sm">{t.name}</p>
                    <p className="text-xs text-muted-foreground">{t.title} · {t.source}</p>
                  </div>
                </motion.div>
              ))}
            </div>
